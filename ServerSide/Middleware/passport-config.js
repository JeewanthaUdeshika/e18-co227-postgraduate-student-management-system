...
export const initalizePassport = (passport) => {
    ...
    const authenticateStaff = async (req, accessToken, refreshToken, profile, done) => {
        // Getting user from the staff data base
        const staff = await StaffDB.findOne({email: profile.email});

        if (!staff){
            return done(null, false, {message: 'There is no user with that email, Contact administrator to add as a staff member'})
        }
        return done(null, staff);
    }

    // For the local login
    passport.use(new LocalStrategy({usernameField: 'email', passwordField: 'password'}, authenticateStudent));
