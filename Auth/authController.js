...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError('You are not logged in . Please log into continue!!', 401)
    );
  }

  // 2) Verification token
  const decoded = await promisify(jwt.verify)(token, process.env.JWT_SECRET);
  //   console.log(decoded); //{id : dfdfgdfdf , iat  :54646  ,exp : 353454}
  ...
  // 4) check if user changed password after issued
